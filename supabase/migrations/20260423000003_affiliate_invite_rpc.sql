-- ============================================================================
-- Task 02 — create_affiliate_invite + resend_affiliate_invite RPCs
-- Sprint: 2026-04-23 affiliate-identity-refactor
--
-- Two SECURITY DEFINER functions called from the invite API route. Both
-- produce atomic writes across affiliate_accounts + diviner_affiliates +
-- affiliate_invites so a half-written invite never exists.
--
-- Error codes surfaced to the route handler:
--   P0001 → affiliate account is blocked platform-wide (→ 403)
--   P0002 → an active/pending/suspended/blocked junction already exists (→ 409)
--   P0006 → legacy pending junction is not in 'pending' status or not owned
--           by the caller (→ 404/409 depending on caller's wrap)
-- ============================================================================

-- ─── 1. create_affiliate_invite ──────────────────────────────────────────────
-- New invite flow: upsert canonical account → insert junction (status=pending)
--                 → insert invite (token_hash, expires_at).
-- Also dual-writes legacy columns (name, email, phone) on diviner_affiliates
-- for back-compat with unmigrated readers.
-- ============================================================================
...
BEGIN
  ...
  -- Preserve existing name if one is set; the invite is a contact prompt,
  ...
  -- Block platform-wide-blocked accounts
  ...
  -- Reject if a junction already exists with any live/suspended/blocked status
  IF EXISTS (
  ...
  -- Create junction (pending). Dual-write legacy identity columns for
  -- pre-Task-06 readers that still consume diviner_affiliates.{name,email,phone}.
...
-- ─── 2. resend_affiliate_invite ──────────────────────────────────────────────
-- Issues a fresh token for an existing invite row. Revokes any prior
-- non-consumed invite on the same junction (there should be at most one live
-- at a time, but we defend in depth).
-- Bumps resent_count.
-- ============================================================================
...
DECLARE
  ...
  v_new_id UUID;
BEGIN
  -- Lock the original row + enforce ownership
  ...
  -- Revoke the original + any other non-consumed invites on the same junction
...
REVOKE EXECUTE ON FUNCTION resend_affiliate_invite FROM PUBLIC;
GRANT EXECUTE ON FUNCTION resend_affiliate_invite TO service_role;


-- ─── 3. resend_affiliate_invite_by_junction ──────────────────────────────────
-- Legacy-pending variant: the 2026-04-23 migration grandfathered junction rows
-- without creating matching affiliate_invites rows (Task 01 § Backfill step 6).
-- This RPC issues a FIRST invite for a junction that has never had one.
-- ============================================================================
...
DECLARE
  ...
  v_new_id UUID;
BEGIN
  -- Look up junction + enforce ownership + status
  SELECT * INTO v_da
  ...
  -- Revoke any existing open invites defensively (there shouldn't be any for
  -- legacy pending rows, but if a prior resend created one, supersede it)
  UPDATE affiliate_invites
     SET revoked_at = NOW()
...
REVOKE EXECUTE ON FUNCTION resend_affiliate_invite_by_junction FROM PUBLIC;
GRANT EXECUTE ON FUNCTION resend_affiliate_invite_by_junction TO service_role;


-- ─── 4. revoke_affiliate_invite ──────────────────────────────────────────────
-- Marks an invite as revoked + delete-or-suspend the junction depending on
-- whether any commissions have been attributed to it yet.
-- ============================================================================
CREATE OR REPLACE FUNCTION revoke_affiliate_invite(
  ...
  invite_id   UUID,
  junction_id UUID,
  junction_action TEXT  -- 'deleted' | 'suspended'
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  -- Mark invite revoked (idempotent if already revoked)
  UPDATE affiliate_invites
     SET revoked_at = COALESCE(revoked_at, NOW())
   WHERE id = p_invite_id;

  -- Decide junction disposition
  SELECT EXISTS (
  ...
  IF v_has_commissions THEN
    -- Can't orphan commission rows — suspend the junction instead
    ...
    v_action := 'suspended';
  ELSE
    -- Safe to delete the pending junction (cascades to other invites)
...
REVOKE EXECUTE ON FUNCTION revoke_affiliate_invite FROM PUBLIC;
GRANT EXECUTE ON FUNCTION revoke_affiliate_invite TO service_role;
