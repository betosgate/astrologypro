...
-- columns named the same as real table columns (invite_id, junction_id,
-- affiliate_account_id, email). Inside the function body any unqualified
-- reference to those names became ambiguous, throwing PG error 42702.
--
-- Fix: add `#variable_conflict use_column` to each function, telling PL/pgSQL
-- to prefer the column interpretation at reference sites. Signatures stay the
-- same (same parameter names, same RETURNS TABLE shape) so the API routes
-- shipped in Task 02 work unchanged.
--
-- CREATE OR REPLACE is sufficient because we're not changing the signature.
-- Idempotent.
-- ============================================================================
