...
-- ─── Helper: resolve auth.uid() → set of junction ids ──────────────────────
-- Inlined into each policy for now. If it becomes a hot path we'll lift
-- it to a SECURITY DEFINER function with a stable plan.
