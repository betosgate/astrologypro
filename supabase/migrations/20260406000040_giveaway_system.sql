...
CREATE INDEX IF NOT EXISTS giveaways_diviner_id_idx ON giveaways(diviner_id);
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      ...
      AND policyname = 'diviners_manage_own_giveaways'
  ) THEN
    EXECUTE $p$
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename  = 'giveaways'
      AND policyname = 'public_read_active_giveaways'
  ) THEN
    EXECUTE $p$
...
CREATE INDEX IF NOT EXISTS giveaway_entries_giveaway_id_idx ON giveaway_entries(giveaway_id, entered_at DESC);
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public'
      AND tablename  = 'giveaway_entries'
      AND policyname = 'diviners_read_own_giveaway_entries'
  ) THEN
    EXECUTE $p$
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename  = 'giveaway_entries'
      AND policyname = 'public_insert_giveaway_entries'
  ) THEN
    EXECUTE $p$
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = 'public'
      AND tablename  = 'giveaway_winners'
      AND policyname = 'diviners_manage_own_winners'
  ) THEN
    EXECUTE $p$
