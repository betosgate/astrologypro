...
- Completion Notes: Implemented. The full required-field set (first_name, last_name, email, phone, gender, state, city, zip, address, occupation, date_of_birth, birth_time) is now wired into src/app/perennial-signup/page.tsx. Phone uses legacyFormatPhone() that produces 123 -> 123, 1234 -> (123) 4, 1234567 -> (123) 456-7, 1234567890 -> (123) 456-7890. Zip is restricted to 5 digits via inputMode=numeric + onChange filter and validated with /^\d{5}$/. Email uniqueness is checked across the household. Legacy relation_type/sub_relation rules: additional members must pick relation_type ("Couple"|"Family") and a sub_relation gated on the type (Couple -> Husband|Wife; Family -> Son|Daughter|Spouse|Partner|Other). Primary member is implicitly relation_type="Self" with sub_relation=null. The 25-field optional questionnaire is rendered for every member, collapsed by default, with all field names matching the spec exactly (lifeAreasFulfilling, workLifeBalance, etc.). NO manual password fields — the form clearly states credentials are generated and emailed after payment.
- Earlier notes: Implemented PARTIAL — primary + additional member forms with first/last/email/relation/dob/birth-time/birth-city/birth-country and an optional questionnaire (intentions/challenges/goals). Email uniqueness validated client-side. The full legacy field set (sub_relation, lifeAreasFulfilling, etc.) is NOT yet wired — task scoped down to keep this commit reviewable. Add the full field set in a follow-up if the legacy set is required for launch.
