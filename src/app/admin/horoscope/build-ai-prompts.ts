...
export function buildAiPrompts(data: any, tab: string, areaOfInquiry?: string, excludedDates?: string) {
  ...
  if (["romantic_forecast_report_tropical_v2", "friendship_report_tropical_v2", "business_partner_v2"].includes(tab)) {
    ...
    const familyMembers = Array.isArray(data.family_members)
      ? data.family_members
      : Array.isArray(data.familyMembers)
        ? data.familyMembers
        : [];
    const familyPersonCharts = Array.isArray(data.family_person_charts)
      ? data.family_person_charts
      : Array.isArray(data.familyPersonCharts)
        ? data.familyPersonCharts
        : [];
    const isFamilyOverview = familyMembers.length > 0;
    ...
    const promptIntro = isFamilyOverview
      ? `This is a family chart overview for ${familyMembers.length} family members. Use every person's birth data and natal chart from family_person_charts in the JSON payload. Do not describe anyone as my romantic partner, friendship partner, business partner, spouse, or any other pair relationship. Make common predictions for the whole family as one shared family system. Keep the response structure exactly the same as requested.`
      : `${b1Str} ${b2Str}`;
    const relationshipSubject = isFamilyOverview
      ? "all family members"
      : `mine and my ${relationshipContext} both`;
    const sharedSubject = isFamilyOverview
      ? "this family chart overview"
      : "this partnership";
    ...
    const compositeUser = tab === "romantic_forecast_report_tropical_v2"
      ? `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate the composite chart of ${sharedSubject}  in {data:[{title:data}]}  exact this format where each data must be atleast 3 sentences with astrological logic with relevance to my question only for each aspect , planet and house on relevant blocks and why you are saying these add an astrologica reason like aspect (with type), house position of planet on each title and data  with astrological data relevant to my data , each title will be heading and data will be context in detail for the title make sure you calculate before response astro analysis must accurate should not change with same data.summery will be mostly generic and recomendtion equal mix of generic and astrological data dont repet that what you have already mentioned in other indexes these could be shorter make sure number content / data on Astrological_aspect is always much more than summery and recomendation to add more here is my birth chart data in json format , try to reffer and mention in your response  `
      : `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate the composite chart of ${sharedSubject}${suffix}`;

    prompts.push({ key: "synastry_horoscope", system: sys, user: `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate the synastry chart of ${sharedSubject}${suffix}`, json: synJson });
...
The user has provided a specific "Area of Inquiry": "${areaOfInquiry || "career"}". Make this the central theme of your interpretation. While you should ground the reading in this context, also incorporate other relevant insights from the chart that support or add nuance to this primary focus. Conclude the response by explicitly summarizing how the various astrological insights tie back to the client’s stated area of inquiry.` : sys;
    ...
    const davisonUser = isBusinessTab
      ? `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate the davison relationship chart of ${sharedSubject} in {data:[{title:data}]}  exact this format where each data must be atleast 5 sentences with astrological logic with relevance to my question only for each aspect , planet and house on relevant blocks and why you are saying these add an astrologica reason like aspect (with type), house position of planet on each title and data  with astrological data relevant to my data , each title will be heading and data will be context in detail for the title make sure you calculate before response astro analysis must accurate should not change with same data.summery will be mostly generic and recomendtion equal mix of generic and astrological data dont repet that what you have already mentioned in other indexes these could be shorter make sure number content / data on Astrological_aspect is always much more than summery and recomendation to add more here is my birth chart data in json format , try to reffer and mention in your response  `
      : tab === "romantic_forecast_report_tropical_v2"
        ? `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate compatibility score or summary of ${sharedSubject}  in {data:[{title:data}]}  exact this format where each data must be atleast 3 sentences with astrological logic with relevance to my question only for each aspect , planet and house on relevant blocks and why you are saying these add an astrologica reason like aspect (with type), house position of planet on each title and data  with astrological data relevant to my data , each title will be heading and data will be context in detail for the title make sure you calculate before response astro analysis must accurate should not change with same data.summery will be mostly generic and recomendtion equal mix of generic and astrological data dont repet that what you have already mentioned in other indexes these could be shorter make sure number content / data on Astrological_aspect is always much more than summery and recomendation to add more here is my birth chart data in json format , try to reffer and mention in your response  `
        : tab === "friendship_report_tropical_v2"
          ? `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate the davison relationship chart of ${sharedSubject}  in {data:[{title:data}]}  exact this format where each data must be atleast 5 sentences with astrological logic with relevance to my question only for each aspect , planet and house on relevant blocks and why you are saying these add an astrologica reason like aspect (with type), house position of planet on each title and data  with astrological data relevant to my data , each title will be heading and data will be context in detail for the title make sure you calculate before response astro analysis must accurate should not change with same data.summery will be mostly generic and recomendtion equal mix of generic and astrological data dont repet that what you have already mentioned in other indexes these could be shorter make sure number content / data on Astrological_aspect is always much more than summery and recomendation to add more here is my birth chart data in json format , try to reffer and mention in your response `
        : `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate Aspect and Conjunction of ${sharedSubject}${suffix.replace("atleast 3 sentences", "atleast 5 sentences")}`;
    const davisonJson = isBusinessTab
      ? {
        mydetails: data.natal_chart_data,
        fiend_details: data.natal_chart_data_p2,
        family_members: familyMembers,
        family_person_charts: familyPersonCharts,
      }
      : tab === "romantic_forecast_report_tropical_v2"
    ...
    const businessMajorAspectsUser = `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate Aspect and Conjunction of ${sharedSubject}  in {data:[{title:data}]}  exact this format where each data must be atleast 5 sentences with astrological logic with relevance to my question only for each aspect , planet and house on relevant blocks and why you are saying these add an astrologica reason like aspect (with type), house position of planet on each title and data  with astrological data relevant to my data , each title will be heading and data will be context in detail for the title make sure you calculate before response astro analysis must accurate should not change with same data.summery will be mostly generic and recomendtion equal mix of generic and astrological data dont repet that what you have already mentioned in other indexes these could be shorter make sure number content / data on Astrological_aspect is always much more than summery and recomendation to add more here is my birth chart data in json format , try to reffer and mention in your response `;
    const businessMajorAspectsJson = {
      mydetails: data.natal_chart_data ?? {},
      fiend_details: data.natal_chart_data_p2 ?? {},
      family_members: familyMembers,
      family_person_charts: familyPersonCharts,
    ...
    prompts.push({
      ...
      user: tab === "business_partner_v2" ? businessMajorAspectsUser : `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate the davison relationship chart of ${sharedSubject}${suffix}`,
      json: tab === "business_partner_v2" ? businessMajorAspectsJson : selfPartnerJson
    });
    prompts.push({ key: "compatibility_score_or_summary", system: sys, user: `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate compatibility score or summary of ${sharedSubject}${suffix}`, json: selfPartnerJson });
...
The user has provided a specific "Area of Inquiry": "${areaOfInquiry || "friendship"}". Make this the central theme of your interpretation. While you should ground the reading in this context, also incorporate other relevant insights from the chart that support or add nuance to this primary focus. Conclude the response by explicitly summarizing how the various astrological insights tie back to the client’s stated area of inquiry.`;
    ...
    const elementalUser = `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate elemental balance of ${sharedSubject} in {data:[{title:data}]} exact this format where each data must be atleast 5 sentences with astrological logic with relevance to my question only for each aspect , planet and house on relevant blocks and why you are saying these add an astrologica reason like aspect (with type), house position of planet on each title and data with astrological data relevant to my data , each title will be heading and data will be context in detail for the title make sure you calculate before response astro analysis must accurate should not change with same data.summery will be mostly generic and recomendtion equal mix of generic and astrological data dont repet that what you have already mentioned in other indexes these could be shorter make sure number content / data on Astrological_aspect is always much more than summery and recomendation to add more here is my birth chart data in json format , try to reffer and mention in your response `;
    ...
    prompts.push({ key: "timing_and_transits", system: sys, user: `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate the timing and transits of ${sharedSubject} in {data:[{title:data}]}  exact this format where each data must be atleast 3 sentences with astrological logic with relevance to my question ("${areaOfInquiry || "friendship"}") only for each aspect , planet and house on relevant blocks and why you are saying these add an astrologica reason like aspect (with type), house position of planet on each title and data  with astrological data relevant to my data , each title will be heading and data will be context in detail for the title make sure you calculate before response astro analysis must accurate should not change with same data.summery will be mostly generic and recomendtion equal mix of generic and astrological data dont repet that what you have already mentioned in other indexes these could be shorter make sure number content / data on Astrological_aspect is always much more than summery and recomendation to add more here is my birth chart data in json format , try to reffer and mention in your response  `, json: timingJson });
    if (tab === "business_partner_v2") {
      ...
      const professionalAlignmentUser = `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate Professional Alignment and Goals of ${sharedSubject}  in {data:[{title:data}]}  exact this format where each data must be atleast 5 sentences with astrological logic with relevance to my question only for each aspect , planet and house on relevant blocks and why you are saying these add an astrologica reason like aspect (with type), house position of planet on each title and data  with astrological data relevant to my data , each title will be heading and data will be context in detail for the title make sure you calculate before response astro analysis must accurate should not change with same data.summery will be mostly generic and recomendtion equal mix of generic and astrological data dont repet that what you have already mentioned in other indexes these could be shorter make sure number content / data on Astrological_aspect is always much more than summery and recomendation to add more here is my birth chart data in json format , try to reffer and mention in your response `;
      const professionalAlignmentJson = {
        mydetails: data.natal_chart_data ?? {},
        fiend_details: data.natal_chart_data_p2 ?? {},
        family_members: familyMembers,
        family_person_charts: familyPersonCharts,
...
The user has provided a specific "Area of Inquiry": "${areaOfInquiry || "marriage"}". Make this the central theme of your interpretation. While you should ground the reading in this context, also incorporate other relevant insights from the chart that support or add nuance to this primary focus. Conclude the response by explicitly summarizing how the various astrological insights tie back to the client’s stated area of inquiry.`;
      const karmicUser = `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate Karmic and supportive Indicators of ${sharedSubject} in {data:[{title:data}]}  exact this format where each data must be atleast 5 sentences with astrological logic with relevance to my question only for each aspect , planet and house on relevant blocks and why you are saying these add an astrologica reason like aspect (with type), house position of planet on each title and data  with astrological data relevant to my data , each title will be heading and data will be context in detail for the title make sure you calculate before response astro analysis must accurate should not change with same data.summery will be mostly generic and recomendtion equal mix of generic and astrological data dont repet that what you have already mentioned in other indexes these could be shorter make sure number content / data on Astrological_aspect is always much more than summery and recomendation to add more here is my birth chart data in json format , try to reffer and mention in your response  `;
      prompts.push({ key: "karmic_and_soulmate_indicators", system: karmicSystem, user: karmicUser, json: selfPartnerJson });
    } else {
      prompts.push({ key: "karmic_and_soulmate_indicators", system: sys, user: `${promptIntro} I have added birth chart details for ${relationshipSubject}; now calculate Karmic and Soulmate Indicators of ${sharedSubject}${suffix}`, json: selfPartnerJson });
    }
  }
  ...
  if (returnTabMap[tab]) {
    ...
    const isUranusOpposition = tab === "uranus_return_v2";
    prompts.push({
      key: returnTabMap[tab],
      system: isUranusOpposition
        ? `give response only in json format as a whole , nothing else asnwer as astrolger not AI BOT. You must authinticate the response with astrologic data and knowledge with zero sentiment and dates and other astrologocal information must be correct in your response and must not change on each api call with same payload and also calculation of uranus opposition must be as per astrological rule, not a general date.The Uranus Opposition is an astrological transit that occurs around the age of 42-44 when Uranus reaches its opposite position in the sky from where it was at your birth. This marks a significant square to your natal Uranus and represents a time of personal upheaval and rebellion. Astrologically, it often manifests as a time when the individual experiences an intense awakening, challenging old structures and beliefs, and urging you to break free from stagnation or conformity. This transit can provoke a crisis of identity, pushing you toward greater individuality and authenticity, often leading to sudden, liberating changes in your life. The opposition occurs when Uranus transits through the sign opposite your natal position, triggering a shift in energy that disrupts the status quo, urging you to embrace freedom and change.. Provide a deeply personalized response as if you are speaking directly to your astrology client in a one-on-one session. Use the language and tone of a trusted Western astrologer offering tailored guidance based on the client’s unique chart. Always interpret the chart using the Placidus house system as the default house_type. Avoid using generic phrases or repeated sentence structures. Each sentence should feel intentionally crafted and distinct, offering fresh insight without duplicating wording from similar interpretations.\n\nThe user has provided a specific "Area of Inquiry": "${areaOfInquiry || "career"}". Make this the central theme of your interpretation. While you should ground the reading in this context, also incorporate other relevant insights from the chart that support or add nuance to this primary focus. Conclude the response by explicitly summarizing how the various astrological insights tie back to the client’s stated area of inquiry.`
        : "give response only in json format as a whole , nothing else asnwer as astrolger not AI BOT  ",
      user: isUranusOpposition
        ? `My Bday is ${data.year} ${getMonthName(data.month)} ${data.day} , time ${convertTo12HourFormat(data.hour, data.min)}  , at  ${city}  I want to know about next uranus opposition from today interested on ${areaOfInquiry || "career"}, relationships and others aspects of life .house system should be whole sign give me chart data first and then detailed content as title and inter pretation in json format but nothing else and each interpretations of each title . example response format should be {\n  "chart_data": {\n    "date_of_birth": "{data}",\n    "time_of_birth": "{data}",\n    "place_of_birth": "{data}",\n   "exact_position_details_of_uranus_at_time_of_birth": "{data}",\n   "next_uranus_opposition": "{date_data}",\n    "house_system": "Whole Sign",\n    "Positions": {\n      "Sun": "{data}",\n      "Moon": "{data}",\n      "Mercury": "{data}",\n      "Venus": "{data}",\n      "Mars": "{data}",\n      "Jupiter": "{data}",\n      "Saturn": "{data}",\n      "Uranus": "{data}",\n      "Neptune": "{data}",\n      "Pluto": "{data}",\n      "North_Node": "{data}",\n      "South_Node": "{data}",\n      "Ascendant": "{data}"\n    }\n  },\n  "title_and_interpretation": {\n    "title": " Upcoming Uranus Opposition Analysis for {data}",\n    "interpretation": {\n      "General": "{data_10}.",\n      "Career": "data.",\n      "Relationships": "data.",\n      "Personal Growth": "{data_10}.",\n      "Health": "{data}",\n      "Family": "{data}",\n      "Social": "{data}",\n      "Spiritual": "{data_10}"\n    }\n  }\n}\n  take json as example structure do not copy content please and replace data with proper detailed related response like real  positions of stars.every intepretation after the title should be with detailed reasoning related to stars/planets and their positions of house keeping significance of uranus in mind and mention both positive and negetives stuffs  with reasoning and interpretation must be very specific as astrologer not general and should sound very confident on observations and keeping age of mine with 3 sentences on each tpoic is must keeping all other plantets positions in calculation too is most important . all {data} and {date_data} must be be populated with real calculated info and it must be a date rage of uranus opposition(this is most critical, it must be perfect, don't respond with a specific date ,if it is not perfect try a date range) . remember all date time data should be in usa date and time format always whereever we have date / time format  {month–day–year order (e.g. July 9, 2024)} and next_uranus_opposition data is must and it also must be a future date for sure later than time now April 15 , 2026 and each date must be exactly perfect and must be in a date rage if you dont find a specific date of uranus opposition after multiple cross check .make sure for each position data is with atleast degrees with house number and impact of the position mentioned  , verify everything as of data from several websites multiple times. All {data_10} must be fulfilled with atleast 3 sentences`
