...
export default function AdminPricingPage() {
  ...
  /* ---- Bottom table: toggle expand item to show plans ---- */
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      {/* ============================================================ */}
      {/* Single unified card: Item editor + Plans                      */}
      {/* ============================================================ */}
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Edit Item &amp; Plans</CardTitle>
          <CardDescription className="text-xs">
            Select an item, edit its details, and manage its plans below.
          </CardDescription>
        </CardHeader>
          ...
          {selected && (
            <>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <Label htmlFor="pricing-key">Item Key</Label>
                  <Input
                    id="pricing-key"
                    value={formItemKey}
                    onChange={(e) => setFormItemKey(e.target.value)}
      ...
      {/* ============================================================ */}
      {/* All pricing items with nested plans                           */}
      {/* ============================================================ */}
      <Card>
        <CardHeader>
        ...
        <CardContent>
          {items.length === 0 ? (
            <p className="text-sm text-muted-foreground italic">No pricing items yet.</p>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b text-left text-xs uppercase text-muted-foreground">
                      ...
                      <td className="py-2 text-center" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-center gap-2">
                          <Switch
                            ...
                            aria-label={`Toggle ${item.item_name} active status`}
                          />
                          {togglingId === item.id && (
                            <Loader2 className="size-3 animate-spin text-muted-foreground" />
                          )}
                    ...
                    {expandedItems.has(item.id) && (
                      <>
                        {itemPlansLoading === item.id ? (
                          ...
                          <tr>
                            <td colSpan={5} className="py-3 pl-10">
                              <p className="text-xs text-muted-foreground italic">No plans for this item.</p>
                            </td>
                          </tr>
                        ) : (
                          (itemPlansCache[item.id] ?? []).map((plan, idx, arr) => (
                            <tr key={plan.id} className={idx === arr.length - 1 ? "border-b" : ""}>
                              <td className="py-1.5 text-right pr-1 text-muted-foreground text-xs">
                                {idx === arr.length - 1 ? "\u2514" : "\u251C"}
                              </td>
                              <td className="py-1.5 font-mono text-xs text-muted-foreground" title={plan.plan_id}>
                                {plan.plan_id}
                              </td>
                              <td className="py-1.5">
                                <span className="text-xs">{plan.display_name}</span>
                                <span className="ml-2 text-xs tabular-nums font-medium">
                                  {plan.currency} {plan.amount.toLocaleString()}
                                </span>
                                {plan.mrp !== null && (
                                  <span className="ml-1 text-xs tabular-nums text-muted-foreground line-through">
                              ...
                              <td className="py-1.5 text-center" onClick={(e) => e.stopPropagation()}>
                                <div className="flex items-center justify-center gap-2">
                                  <Switch
                                    ...
                                    disabled={planTogglingId === plan.id}
                                    onCheckedChange={() => handleTogglePlan(plan)}
                                    aria-label={`Toggle ${plan.display_name}`}
                                    className="scale-75"
                                  />
                                  {planTogglingId === plan.id && (
                                    <Loader2 className="size-3 animate-spin text-muted-foreground" />
