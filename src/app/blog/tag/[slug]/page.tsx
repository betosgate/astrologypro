...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!tag) return {};
  const title = `#${tag.name} | AstrologyPro Blog`;
  const description = `Browse all articles tagged "${tag.name}" on the AstrologyPro blog.`;
  return {
    title,
    description,
...
export default async function TagPage({ params, searchParams }: Props) {
  ...
  const [tag, categories, { posts, nextCursor }] = await Promise.all([
    getTagBySlug(slug),
    getAllCategories(),
    getPublishedPosts({ tagSlug: slug, limit: 12, cursor }),
  ]);
          ...
          <div className="mx-auto max-w-5xl">
            <div className="mb-10">
              ...
              <div className="flex items-center gap-3">
                <span className="rounded-full border border-purple-500/30 bg-purple-500/10 px-3 py-1 text-sm font-semibold text-purple-400">
                  #{tag.name}
                </span>
              </div>
              <h1 className="mt-3 text-3xl font-bold text-[#f5f0e8] sm:text-4xl">
                Tagged: {tag.name}
              </h1>
            </div>
              ...
              <>
                ...
                {nextCursor && (
                  <div className="mt-12 text-center">
                    <Link
                      href={`/blog/tag/${slug}?cursor=${nextCursor}`}
