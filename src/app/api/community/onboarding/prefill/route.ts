...
export async function GET() {
  try {
    ...
    const { data: member, error: memberErr } = await supabase
      ...
      .select(
        "id, email, full_name, first_name, last_name, phone, gender, date_of_birth, birth_time, birth_city, birth_country, state, city, zip, address, relationship_status, relation_type, intake_data, membership_status, membership_type, plan_type, onboarding_completed"
      )
      .eq("user_id", user.id)
      .maybeSingle();
