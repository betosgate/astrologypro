...
export async function GET(req: NextRequest) {
  ...
  const isForecast = sp.get("is_forecast") ?? "";
  ...
  let query: any = admin
    .from("mundane_events")
    .select(
      "id, title, event_type, event_date, event_time, location, is_forecast, forecast_confidence, primary_entity_id, is_public, tags, created_at",
      { count: "exact" }
    );

  if (entityId) query = query.eq("primary_entity_id", entityId);
  if (type) query = query.eq("event_type", type);
  if (dateFrom) query = query.gte("event_date", dateFrom);
  if (dateTo) query = query.lte("event_date", dateTo);
  if (isForecast === "true") query = query.eq("is_forecast", true);
  if (isForecast === "false") query = query.eq("is_forecast", false);
...
export async function POST(req: NextRequest) {
  ...
  const body = await req.json() as {
    ...
    astrological_factors?: Record<string, unknown>;
  ...
  const { title, event_type, description, event_date, event_time, location, primary_entity_id, is_forecast, forecast_confidence, astrological_factors, outcome_notes, source, tags, is_public } = body;
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      astrological_factors: astrological_factors ?? null,
