...
export async function GET(req: NextRequest) {
  ...
  const totalEntities = allRows.length;
  ...
  const entities = (data ?? []).map((e: {
    id: string; name: string; entity_type: string; region: string | null;
    flag_emoji: string | null; is_active: boolean; created_at: string;
  }) => ({
    ...e,
