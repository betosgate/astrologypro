...
export async function POST(req: NextRequest) {
  ...
  const { sign, decan, title, video_url, pdf_url, description, is_active } = body;
  ...
  const { data, error } = await admin
    .from("decan_media")
    .insert({ sign, decan: Number(decan), title, video_url: video_url ?? null, pdf_url: pdf_url ?? null, description: description ?? null, is_active: is_active ?? true })
    .select()
    .single();
