 ...
 * supabase/migrations/20260428000001_landing_page_cleanup_destructive.sql.
 *
 * ⚠️  DESTRUCTIVE. NOT REGISTERED in src/lib/db/migrations.ts so the on-demand
 *     runner at /admin/db/migrations cannot invoke it. It must be applied
 *     through a controlled deploy process — see
...
-- STEP 4 — Drop the container table.
DROP TABLE IF EXISTS service_landing_pages CASCADE;

-- STEP 5 — Drop publish_status on diviner_services.
...
-- STEP 7 — Rename the sections table.
ALTER TABLE service_landing_page_sections
