...
export function WhatsIncludedEditor({ section }: EditorProps) {
  const raw = (section.draft_content_json ?? section.content_json) as Record<string, unknown>;
  const init: WhatsIncludedContent = {
    heading: "What's Included",
    subtitle: "",
    ...raw,
    items: Array.isArray(raw.items) ? (raw.items as string[]) : [],
  };
  const { values, set, revert, save, saving } = useEditorForm<WhatsIncludedContent>(section, init);
